...
* Прописал в ansible.cfg использование в качестве инвентори-файла gce.py;
...
### Основное задание
* Установил Vagrant и описал в нем текущую инфрастурктуру;
* Проверил создание виртуальных машин;
* Описал провижининг в Vagrantfile, проверил работу провижененра;
* Добавил роль по установке python;
* Доработал роль db для установки MongoDB;
* Вынес таски в отдельные файлы;
* Доработал роль app по аналогии с db;
* Изучил работу с инвентори в Vagrant, параметризировал роли через инвентори;
* Изменил роли для использования пользователя из переменной;
* Переопределил переменную пользователя через extra_vars;
* Настроил virtualenv, установил Molecule, Ansible, Testinfra в созданном вирутальном окружении;
* Провел инициализацию molecule для роли db;
* Добавил тесты для роли db;
* Настроил параметры создания вирутальной машины в конфиге molecule;
* Создал машину через molecule, применил плейбук, прогнал тесты;
* Выполнил самостоятельное задание:
* * Добавил тест для проверки прослушивания порта 27017 для роли db;
* * Использовал в плейбуках packer_db.yml и packer_app.yml роли db и app c использованием нужных тегов.

### Задание со * (1)
* Настроил парамерты роли установки nignx через extra_vars.

### Задание со * (2)
* Вынес роль db в отдельный репозиторий https://github.com/yusorokin/infra_db_role;
* Настроил тестирование роли в TravisCI с помошью molecule c драйвером gce;
* Настроил шаблон машины molecule, плейбука для конверджа, тесты molecule;
* Настроил подключение драйвера gce molecule через сервиснуд учетную запись gce;
* Добавил бейдж билда;
* Настроил оповещение тревиса в slack.
